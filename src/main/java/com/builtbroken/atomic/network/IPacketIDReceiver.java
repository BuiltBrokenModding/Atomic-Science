...
public interface IPacketIDReceiver
{
     ...
     * @param position - location the packet should be read at, will return player location for
     *                        world packets without postion data, and null if non-world packets
     * @return true if the packet should be read
     */
    default boolean shouldReadPacket(EntityPlayer player, IPosWorld position, IPacket packet)
    {
        return true;
    }
}
